...
import com.civicissues.exception.InvalidActionException;
...
public class AdminController {
    ...
    public ResponseEntity<ApiResponse<Void>> deactivate(@PathVariable Long userId) {
        if(userId==1) {
        	throw new InvalidActionException("Cant deactivate Super Admin");
        }  
  ...
  @PreAuthorize("hasRole('SUPER_ADMIN')")
    ...
    public ResponseEntity<ApiResponse<Void>> activate(@PathVariable Long userId) {
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(true, "User activated successfully", null));
    }
